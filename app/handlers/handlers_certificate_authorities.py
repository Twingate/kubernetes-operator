...
from app.crds import CertificateAuthoritySpec, CertificateAuthorityType
...
def _reconcile_certificate_authority(body, spec, logger, memo, patch):
    ...
    # The backend (and this operator) only manage X509 CAs today; SSH CAs are a
    # future addition. Fail loudly rather than silently ignoring.
    if ca_spec.type != CertificateAuthorityType.X509:
        raise kopf.PermanentError(
            f"Unsupported certificate authority type: {ca_spec.type}. "
            "Only X509 is supported."
        )
    ...
    # is reconciled by re-creating it (a new backend id), not by updating in place.
